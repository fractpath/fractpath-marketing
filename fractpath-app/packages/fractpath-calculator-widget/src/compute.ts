import type { CalcInput, CalcOutput, ScheduleRow, SettlementCase, Summary } from "./types";

export const TERMS_VERSION = "fractpath-terms-v1.0";
export const SCHEMA_VERSION = "1";

function validateInputs(raw: Record<string, unknown>): CalcInput {
  ...
  if (
    !Number.isFinite(fractional_percent) ||
...
function buildSchedule(input: CalcInput): ScheduleRow[] {
...
function buildSummary(input: CalcInput, schedule: ScheduleRow[]): Summary {
  const { home_value, fractional_percent, term_years, appreciation_rate, discount_rate } =
    input;
  const frac = fractional_percent / 100;
  const buyAmount = home_value * frac;
...
export function computeDeal(rawInputs: Record<string, unknown>): CalcOutput {
