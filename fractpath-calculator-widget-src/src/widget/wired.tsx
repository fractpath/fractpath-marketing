...
import { buildShareSummary, buildSavePayload } from "./snapshot.js";
...
export function WiredCalculatorWidget(props: FractPathCalculatorWidgetProps) {
  ...
  // App mode hydration
  useEffect(() => {
    if (!isApp || !initialSnapshot) return;

    const dt = initialSnapshot.deal_terms ?? {};
    const asn = initialSnapshot.assumptions ?? {};

    setHomeValue(
      typeof dt.property_value === "number" ? dt.property_value : homeValue,
    );
    setInitialBuyAmount(
      typeof dt.upfront_payment === "number"
        ? dt.upfront_payment
        : initialBuyAmount,
    );
    setTermYears(
      Math.max(
        ...
        Math.min(
          30,
          Math.round((dt.number_of_payments ?? termYears * 12) / 12),
        ),
      ),
    );
    setGrowthRatePct(
      Math.max(
        0,
        Math.min(20, (asn.annual_appreciation ?? growthRatePct / 100) * 100),
      ),
    );

    setDirty(false);
  }, [isApp, initialSnapshot]);
  ...
  const outputs = useMemo(() => {
    if (isApp && initialSnapshot && !dirty)
      return initialSnapshot.outputs as any;

    // Flatten DealTerms for computeScenario / computeDeal
    const flatInputs = {
      homeValue,
      initialBuyAmount,
      termYears,
    ...
    return computeScenario(flatInputs);
  }, [
    isApp,
    initialSnapshot,
    dirty,
  ...
  // --- Canonical Save & Continue for app resume ---
  const handleSaveContinue = useCallback(async () => {
    onEvent?.({ type: "save_continue_clicked", persona });
    if (!onDraftSnapshot) return;

    const canonicalSnapshot: FullDealSnapshotV1 = {
      contract_version: "1.0.0",
      schema_version: "1.0.0",
      deal_terms: {
        property_value: homeValue,
        upfront_payment: initialBuyAmount,
        monthly_payment: (growthRatePct / 100) * homeValue,
        number_of_payments: Math.round(termYears * 12),
        floor_multiple: 1,
        ceiling_multiple: 1,
      ...
      assumptions: { annual_appreciation: growthRatePct / 100 },
      outputs,
      now_iso: new Date().toISOString(),
      created_at: new Date().toISOString(),
    };

    onDraftSnapshot(canonicalSnapshot);

    const w = window as any;
    if (typeof w.__fractpath_saveSnapshot === "function") {
      w.__fractpath_saveSnapshot(canonicalSnapshot);
    }
  }, [
    persona,
    homeValue,
