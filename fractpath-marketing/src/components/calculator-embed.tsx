    ...
    async (e: FormEvent<HTMLFormElement>) => {
      ...
      try {
        const res = await fetch("/api/share", {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            email, // route.ts expects `email`
            summary: gate.summary, // route.ts expects `summary`
          }),
        });
        ...
        if (data?.share_token || data?.ok) {
          ...
          const msg =
            typeof data?.error === "string" && data.error.trim()
              ? data.error.trim()
              : `Share failed (status ${res.status})`;

          console.warn("[share] failure:", msg, { to_email: email, data });
          setGate({
            step: "share_error",
            message: msg,
